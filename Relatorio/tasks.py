...
def run_sync_fracttal(repeat_seconds=None): 
...
@background(schedule=10)
def run_update_active_os(repeat_seconds=None): 
...
def run_validate_os_sequence():
    ...
    try:
        ...
        if now >= run_time:
             ...
             logger.info("Tarefa validate_os_sequence concluída com sucesso.") 
        else:
        ...
        run_validate_os_sequence(schedule=next_run_time) 

    except Exception as e:
        print(f"[{timezone.now().strftime('%Y-%m-%d %H:%M:%S')}] background_task: ERRO ao chamar/reagendar validate_os_sequence: {e}")
        logger.error(f"Erro ao executar validate_os_sequence: {e}", exc_info=True) 
        try:
